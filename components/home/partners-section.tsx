...
export function PartnersSection() {
  return (
    <section className="bg-(--forest-subtle) pt-56 pb-20">
      <div className="max-w-8xl mx-auto w-full px-4 sm:px-6 lg:px-8 xl:px-10">
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-base leading-6 text-(--text-fg-muted)">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-4">
          {partnerSlots.map((slot) => (
            <div
              key={slot}
              className="flex h-14 w-[184px] items-center justify-center rounded-xl border border-(--border-subtle) bg-white"
            >
              <span className="text-[11px] font-semibold tracking-[0.14em] text-(--text-fg-subtle)">
                PARTNER LOGO
              </span>
            </div>
          ))}
