                ...
                <span className="relative flex size-6 shrink-0 items-center justify-center sm:size-7">
                  {isCurrent && !reduce ? (
                    <motion.span
                      ...
                      className="ring-leaf-200 ring-offset-background bg-forest-900 absolute inset-0 rounded-full ring-2 ring-offset-2"
                      transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1] }}
                    />
                  ) : null}
                  <span
                    className={cn(
                      "relative z-10 flex size-full items-center justify-center rounded-full text-[11px] font-semibold transition-colors sm:text-xs",
                      isComplete && "bg-leaf-500 text-white",
                      isCurrent &&
                        reduce &&
                        "bg-forest-900 ring-leaf-200 ring-offset-background text-white ring-2 ring-offset-2",
                      isCurrent && !reduce && "text-white",
                      !isCurrent &&
                        !isComplete &&
