...
live community application receipt in Apple Mail dark mode). The shell declared
`color-scheme: light dark` and shipped an `emailColorSchemeStyles` block, but
that block only locked the masthead, CTA, and footer for dark mode — the body
card and its text (`.email-body`, `.email-display`, `.email-lead`, and the
class-less sign-off) had no dark overrides. Apple Mail darkened the un-locked
white card while the inlined dark text stayed dark, so exactly the un-covered
body copy went unreadable.

**Decision: these templates are light-only.** The client wanted a clean,
reliable render over true dark support, so instead of completing the dark
coverage we opted out of dark mode: `color-scheme` / `supported-color-schemes`
are now `light`, and the redundant `emailColorSchemeStyles` block was deleted.
Color tokens (`footerText`, `mastheadMuted`, …) stay — they are used as inline
styles — and both responsive `@media (max-width)` blocks stay. Do not re-add
`prefers-color-scheme` / `[data-ogsc]` overrides unless the light-only decision
is explicitly revisited. Caveat: Outlook (Windows/web) ignores `color-scheme`
and may still invert, but now uniformly rather than half-broken. Shared shell,
so all five templates are covered. Files: `lib/email/components/email-shell.tsx`,
`lib/email/styles.ts`.

## Turnstile widget made always-visible (2026-07-24)

Fixed a production UX trap on the public forms (reported on the live membership
dialog). The shared `TurnstileWidget` used `appearance: "interaction-only"`,
which kept the widget invisible for low-risk visitors and minted a token
silently. When a visitor hit Submit before that token arrived, the client guard
in `use-public-form-submit` showed "Please complete the verification step" with
no widget on screen to complete — the "it tells me to verify but nothing shows"
report. Switched to `appearance: "always"` so the checkbox/status card is
visible on all six forms, and mapped the widget `theme` to the existing `tone`
prop (dark card on dark surfaces like the footer newsletter). Deliberately did
**not** hard-disable Submit on an empty token: the widget's error-fallback path
("Try again / email us") must stay reachable, and gating would lock out anyone
whose widget failed to load. Single change in `components/shared/turnstile-widget.tsx`.

Note: this is UX-only. The separate live "We couldn't send your message right
now" failure is a Resend account/key mismatch in Vercel Production (the
`RESEND_API_KEY` / `EMAIL_FROM` must belong to the same account where
`iproduceafrica.com` is verified) and is unaffected by this change.
...
| 2026-07-23 | Codex  | Deployed the final required-reference schema, confirmed the Production deployment is Ready, and registered the apex and `www` custom domains on the Vercel project without changing public DNS. Recorded the live pre-cutover state (old apex IP, `www` alias, four-hour TTL, no CAA) so the domain remains behind its independent client go/no-go. Updated the existing Notion checklist in place; Mailchimp remained delegated and untouched.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Vercel project domains; `docs/{production-closeout-runbook,status-board,implementation-log}.md`; Notion `iProduce Dev Notes`                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `pnpm test` (41/41); `pnpm lint`; `pnpm typecheck`; `pnpm format:check`; `pnpm build`; `git diff --check`; Production route smoke checks                                                                                                     | Repo and dataset work complete — external Sanity webhook settings, Turnstile hostname allowlist, DNS launch approval/cutover, final browser QA, and handover remain.                    |
| 2026-07-23 | Codex  | Added a copy-paste Mailchimp campaign template derived from the public subscriber email system. The standalone HTML keeps the iProduce masthead, editorial typography, highlight panel, CTA, responsive treatment, and dark-green footer while adding Mailchimp-editable regions and the required address, preferences, unsubscribe, archive, and year merge tags. This is an optional campaign-design handoff asset and does not alter newsletter signup or double-opt-in delivery.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `docs/mailchimp-campaign-template.html`, `docs/implementation-log.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Prettier HTML check; `git diff --check`                                                                                                                                                                                                      | Complete                                                                                                                                                                                | Paste the full HTML into Mailchimp's custom-code builder, preview desktop/mobile, and send one test campaign before saving it as the client's reusable template.                                                                                                                                                                                                                                                                                             |
